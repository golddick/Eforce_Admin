...
function Signup() {
    ...
    const handleSubmit = async (e) => {
        ...
        const confirmPasswordValue = password_confirmation.current.value;

        if (confirmPasswordValue !== passwordValue) {
            setErrorMessage("Passwords don't match!");
        } else if (!passwordPattern.test(passwordValue)) {
            setErrorMessage("Password must have at least 8 characters, including at least one uppercase letter, one lowercase letter, one special character, and one number.");
        } else {
            const user = {
            ...
            try {
                await axios.post("https://project-e-force-backend-v2.onrender.com/api/v1/authentication/signup?type=vendor", user);
                ...
                <div className="containerBody">
                    ...
                    <div className="formBody">
                        <form action="" className='form'>
                            ...
                            <input type="email" id='email' className='BodyFormInput' ref={email}  required/>
                            <div className='inputIcon'>@</div>
                            </div>
                            <div className="formFiled">
                                <label htmlFor="password" className='labelText'>password</label>
                            <input type="password" id='password'  className='BodyFormInput' ref={password} required/>
                            <div><CiUnlock className='inputIcon'/></div>
                            </div>
                            <div className="formFiled">
                                <label htmlFor="password_confirmation" className='labelText'>Confirm password</label>
                            <input type="password" id='password_confirmation'  className='BodyFormInput' ref={password_confirmation} required/>
                            <div><CiUnlock className='inputIcon'/></div>
                            </div>
                        </form>
                            {
                                errorMessage &&   (
                                  ...
                                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 28 27" fill="none">
                                    ...
                                    <span className='warningText'>{errorMessage}</span>
                                    </div>
                                )
                            }
                                             {successMessage && (
                        ...
                        <div className="forgotP" onClick={NavtoLogin}>
                            <div className="restP" >
                            ...
                            <span className='resetText'>Login</span>
                            </div>
                        </div>
